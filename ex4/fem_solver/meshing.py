...
def get_mesh(mesh_size, width, height, radius, mesh_size_circle=None):
    gmsh.initialize()
    ...
    # Add circle (hole) points with a smaller characteristic length for denser mesh
    center_x = width / 2
    center_y = height / 2
    ...
    # Define the circle as two arcs (to form a full circle)
    ...
    gmsh.model.geo.addPlaneSurface([outer_loop, circle_loop])
...
if __name__ == "__main__":
    ...
    node, element, outline_indices = get_mesh(mesh_size=mesh_size, width=width, height=height, radius=radius,
