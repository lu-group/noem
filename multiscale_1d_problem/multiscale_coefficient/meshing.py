...
def get_fem_mesh(L, ele_num, k=1, c=1):
    ...
    ele_loc = [0.5 * (node[i] + node[i + 1]) for i in range(len(element))]
    ele_loc = np.array(ele_loc)
    k_list = 0.5 * np.sin(2 * np.pi * ele_loc) + 0.8
    c_list = np.zeros(len(element)).tolist()
    f_list = 0.5 * L / ele_num
...
def get_don_mesh(L, ele_num, don_segment, k=1, c=1):
    ...
    for i in range(len(don_segment)):
        ...
        for j in range(len(node)):
            x = node[j]
            # Delete the node if it is in the segment
            if tx1 < x < tx2:
    ...
    fem_ele_loc = 0.5 * (node[fem_element[:, 0]] + node[fem_element[:, 1]])
    k_list = 0.5 * np.sin(2 * np.pi * fem_ele_loc) + 0.8
    c_list = np.zeros(len(fem_element)).tolist()
    f_list = 0.5 * L / ele_num
...
if __name__ == '__main__':
    ...
    #     x1 = node[mesh[0]]
    #     x2 = node[mesh[1]]
    #     plt.plot([x1, x2], [U[mesh[0]], U[mesh[1]]], 'b')
    # for mesh in don_mesh:
    #     x = np.linspace(0.01, 0.99, 51) + node[mesh[0]]
    #     T = get_don_prediction(U[mesh[0]], U[mesh[1]], x, don_element.DON_info.net)
    #     x = x.tolist()
    #     T = T.tolist()
    #     plt.plot(x, T, 'r--')
    plt.plot(x, U_pred, 'r')
