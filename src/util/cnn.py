...
class CNN(nn.Module):
    def __init__(self, activation1=F.relu, activation2=F.tanh, outputsize=128):
        ...
        # Define the layers
        self.reshape = nn.Unflatten(1, (1, 40, 40))  # Reshape input to (1, 20, 20)
        self.conv1 = nn.Conv2d(1, 128, kernel_size=5, stride=2)  # First convolution layer
        self.conv2 = nn.Conv2d(128, 128, kernel_size=5, stride=2)  # Second convolution layer
        self.conv3 = nn.Conv2d(128, 128, kernel_size=5, stride=2)  # Second convolution layer
        self.flatten = nn.Flatten()  # Flatten the output
        self.fc1 = nn.Linear(512, outputsize)  # First fully connected layer
