...
def solver(node, mesh, T_BC, T_nodeid, q_BC, q_nodeid, k=1, element_type='quad4'):
    if element_type == 'quad4':
        import heat_transfer.fem_solver.element.quaelement as element
        ele_node_num = 4
    elif element_type == 'tri3':
        import heat_transfer.fem_solver.element.trielement as element
        ele_node_num = 3
    ...
    for ele in mesh:
        ...
        ele_k = element.get_elek(ele_coords, k)
...
if __name__ == '__main__':
    ...
    T_nodeid = left_nodeid + top_nodeid
    T_BC = np.zeros(len(T_nodeid))

    q_nodeid = right_nodeid
    q_mag = 0
    q_BC = np.zeros(len(right_nodeid))  # Evenly distribute heat flux

    q_nodeid += bottom_nodeid
    tq_BC = np.zeros(len(bottom_nodeid)) + 10 * x_len / num_x
    tq_BC[0] /= 2
    tq_BC[-1] /= 2
    q_BC = np.concatenate((q_BC, tq_BC))


    # T_nodeid = left_nodeid + right_nodeid + top_nodeid + bottom_nodeid
    # T_BC = np.zeros(len(T_nodeid))
    # q_BC = np.array([])
    # q_nodeid = np.array([])
    #
    T = solver(node, mesh, T_BC, T_nodeid, q_BC, q_nodeid, k=1)
    ...
    #     plt.text(n[0], n[1], str(idx), fontsize=8)
    # # Plot the mesh
    # for ele in mesh:
    #     for i in range(4):
    #         plt.plot([node[ele[i]][0], node[ele[(i + 1) % 4]][0]],
    #                  [node[ele[i]][1], node[ele[(i + 1) % 4]][1]], 'k-')
    #     # Print the text of the element idx and its node idx at the corresponding centroid
    #     centroid = np.mean([node[ele[i]] for i in range(4)], axis=0)
    #     plt.text(centroid[0], centroid[1], str(ele), fontsize=8)
