...
import darcy_flow.linear_permeability.visualization as visualization
import os, torch
import darcy_flow.fem_solver.fem_solver as fem_solver
from darcy_flow.fem_solver.don_solver import one_solver
...
def get_K():
    ...
    randon_field_K = np.load(BASE_DIR / "K_field.npz")["K"]
...
def get_k_list(fem_ele_loc, randon_field_K, don_ele_loc):
    ...
    for i in range(len(fem_ele_loc)):
        ...
        if np.any(np.all(don_ele_loc == tele_loc, axis=1)):
            ...
            k_list.append(1)
    return k_list
def plot_quadrilateral_mesh(mesh_coor, value):
    colormap = "rainbow"
    fontsize = 12
    ...
    verts = mesh_coor
    collection = PolyCollection(verts, array=np.array(value), cmap='viridis')
    ...
    plt.savefig(results_dir / "K_field.png")
def visual_K(don_ele_loc, randon_field_K, node, mesh, mesh_size=0.05):
    ele_loc = get_eleloc(node, mesh)
    don_ele_loc[:, 0] = don_ele_loc[:, 0] + 0.5
    don_ele_loc[:, 1] = don_ele_loc[:, 1] + 0.5
    k_list = np.append(randon_field_K, np.ones(len(ele_loc)), axis=0)
    mesh_coors = []
    for i in range(len(don_ele_loc)):
        tmesh_coors = [[don_ele_loc[i][0] - mesh_size / 2, don_ele_loc[i][1] - mesh_size / 2],
                       [don_ele_loc[i][0] + mesh_size / 2, don_ele_loc[i][1] - mesh_size / 2],
                       [don_ele_loc[i][0] + mesh_size / 2, don_ele_loc[i][1] + mesh_size / 2],
                       [don_ele_loc[i][0] - mesh_size / 2, don_ele_loc[i][1] + mesh_size / 2]]
        mesh_coors.append(tmesh_coors)
    for i in range(len(mesh)):
        tmesh_coors = []
        for j in range(len(mesh[i])):
            tmesh_coors.append(node[mesh[i][j]])
        mesh_coors.append(tmesh_coors)
    plot_quadrilateral_mesh(mesh_coors, k_list)

def run(net_name = "deeponet_linear_darch.pth", results_name=None):
    x_len = y_len = 2.0
    num_x = num_y = 40
    ...
    T_BC = np.load(BASE_DIR / "T_BC.npz")['T_BC']
    ...
    for i in range(len(q_nodeid)):
        q_nodeid[i] = transfer_nodeid(q_nodeid[i], existed_orignal_nodeid, existed_new_nodeid)
    ...
    visual_K(don_ele_loc, randon_field_K, don_node, don_mesh)

    from darcy_flow.fem_solver.element import don_element as don_element
    is_branch_zeromean = False
    don_element.DON_info.initialize(net_name=net_name, x_len=1, y_len=1, r=0, is_zeromean=is_branch_zeromean)
    one_T = one_solver(node=don_node, mesh=don_mesh, T_BC=T_BC, T_nodeid=T_nodeid, q_BC=q_BC, q_nodeid=q_nodeid,
                       T_oneBC_nodeid_list=[T_oneBC_nodeid], don_element=don_element, k_fem_list=np.ones(len(don_mesh)),
                       k_don_list=[randon_field_K], element_type='qua4', is_logging=True)
    fem_node = fem_solver.get_node(x_len, y_len, num_x, num_y)
    fem_mesh = fem_solver.get_mesh(num_x, num_y)
    left_nodeid = [i * (num_x + 1) for i in range(num_y + 1)]
    right_nodeid = [(i + 1) * (num_x + 1) - 1 for i in range(num_y + 1)]
    ...
    fem_T = fem_solver.solver(node=fem_node, mesh=fem_mesh, T_BC=fem_T_BC, T_nodeid=fem_T_nodeid, q_BC=q_BC,
                              q_nodeid=q_nodeid, k_list=fem_k_list, element_type='quad4')
    ...
    meanT = np.mean(don_BC_T)
    don_BC_T = don_BC_T - meanT
    for i in range(len(don_visual_points)):
        don_visual_T[[i]] = visualization.get_don_results(net=don_element.DON_info.net,
                                                          trunk_input=torch.tensor(don_visual_points[i]),
                                                          branch_input1=randon_field_K, branch_input2=don_BC_T)
    don_visual_T = don_visual_T + meanT
    ...
    error = visualization.plot_2d_diff_contour(node1=np.array(fem_node), results1=fem_T,
