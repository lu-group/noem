...
class creatDataSet(Dataset):
    def __init__(self, data_path, sample_name, sample_num=None, trunk_sample_num=None,
                 branch_input1_min=None, branch_input1_max=None,
                 branch_input2_min=None, branch_input2_max=None,
                 trunk_input_min=None, trunk_input_max=None,
                 output_min=None, output_max=None, device=None):
        if device is None:
            device = 'cuda' if torch.cuda.is_available() else 'cpu'

        branchinput1_name = sample_name["branchinput1"]
        branchinput2_name = sample_name["branchinput2"]
        trunk_name = sample_name["trunk"]
        label_name = sample_name["label"]

        # Load the data
        branch_input1_filename = data_path + branchinput1_name
        branch_input1_data = pd.read_csv(branch_input1_filename, sep=",")
        self.branch_input1_tensor = torch.tensor(np.array(branch_input1_data, dtype=np.float32))
        branch_input1_data = None
        branch_input2_filename = data_path + branchinput2_name
        branch_input2_data = pd.read_csv(branch_input2_filename, sep=",")
        self.branch_input2_tensor = torch.tensor(np.array(branch_input2_data, dtype=np.float32))
        branch_input2_data = None
        ...
        if trunk_sample_num is None:
            ...
            lable_index_list = []
            for i in range(self.branch_input1_tensor.size(0)):
        ...
        if sample_num is not None:
            self.branch_input1_tensor = self.branch_input1_tensor[:sample_num, :]
            self.branch_input2_tensor = self.branch_input2_tensor[:sample_num, :]
        ...
        self.branch_input1_tensor, self.branch_input1_min, self.branch_input1_max = self.min_max_scaler(self.branch_input1_tensor, branch_input1_min, branch_input1_max)
        self.branch_input2_tensor, self.branch_input2_min, self.branch_input2_max = self.min_max_scaler(self.branch_input2_tensor, branch_input2_min, branch_input2_max)
        ...
        self.branch_input1_tensor = self.branch_input1_tensor.to(device)
        self.branch_input2_tensor = self.branch_input2_tensor.to(device)
    ...
    def __getitem__(self, idx):
        ...
        return self.branch_input1_tensor[idx], self.branch_input2_tensor[idx], self.label_tensor[idx * self.trunk_sample_num: (idx + 1) * self.trunk_sample_num], self.label_norm[idx]

    def __len__(self):
        return self.branch_input1_tensor.size(0)
...
def train(net, batch_size, max_epoch, TOL,
    ...
    branch_input1_min, branch_input1_max = train_dataset.branch_input1_min, train_dataset.branch_input1_max
    branch_input2_min, branch_input2_max = train_dataset.branch_input2_min, train_dataset.branch_input2_max
    ...
    net.config["branch_input1_min"] = branch_input1_min
    net.config["branch_input1_max"] = branch_input1_max
    net.config["branch_input2_min"] = branch_input2_min
    net.config["branch_input2_max"] = branch_input2_max
    ...
    test_dataset = creatDataSet(data_path, sample_name=test_sample_name, sample_num=None, trunk_sample_num=test_trunk_sample_num,
                                branch_input1_min=branch_input1_min, branch_input1_max=branch_input1_max,
                                branch_input2_min=branch_input2_min, branch_input2_max=branch_input2_max,
    ...
    for epoch in pbar:
        train_loss_list = []
        for idx, (train_branch_input1, train_branch_input2, train_label, lable_norm) in enumerate(train_loader):
            train_label = train_label.view(-1, train_label.size(2))

            opt.zero_grad()

            predict_y = net.forward_branch_trunk_fixed(branch_input_list=[train_branch_input1, train_branch_input2],
                                                       trunk_input=train_trunk_input)
            opt.zero_grad()
            if loss_type == "MSE":
                loss = loss_fn(predict_y, train_label)
            elif loss_type == "L2":
                loss = relative_l2_loss(predict_y=predict_y, train_label=train_label, trunk_num=train_trunk_sample_num, label_norm=lable_norm, is_threshold=False)
            else:
                raise ValueError("The loss type is not supported.")
        ...
        for idx, (test_branch_input1, test_branch_input2, test_label, lable_norm) in enumerate(test_loader):
            test_label = test_label.view(-1, test_label.size(2))
            predict_y = net.forward_branch_trunk_fixed(branch_input_list=[test_branch_input1, test_branch_input2],
                                                       trunk_input=test_trunk_input)
            if loss_type == "MSE":
                loss = loss_fn(predict_y, test_label)
            elif loss_type == "L2":
                loss = relative_l2_loss(predict_y=predict_y, train_label=test_label, trunk_num=test_trunk_sample_num, label_norm=lable_norm, is_threshold=False)
            else:
                raise ValueError("The loss type is not supported.")
